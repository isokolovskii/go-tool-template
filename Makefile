...
.PHONY: build
build:
	go build -ldflags "-s -w -X github.com/library_owner/library_name/internal/version.commit=$(COMMIT_HASH)" -o library_name

.PHONY: build-with-coverage
build-with-coverage:
	go build -cover -ldflags "-s -w -X github.com/library_owner/library_name/internal/version.commit=$(COMMIT_HASH)" -o library_name
